...
class CompilerDaemonTest : KotlinIntegrationTestBase() {
    ...
    fun testDaemonReplLocalEvalNoParams() {
        withDaemon { daemon ->
            withDisposable { disposable ->
                val repl = KotlinRemoteReplCompilerClient(disposable, daemon!!, null, CompileService.TargetPlatform.JVM,
                                                          emptyArray(),
    ...
    fun testDaemonReplAutoshutdownOnIdle() {
        withFlagFile(getTestName(true), ".alive") { flagFile ->
            ...
            val replCompiler = KotlinRemoteReplCompilerClient(disposable, daemon!!, null, CompileService.TargetPlatform.JVM,
                                                              emptyArray(),
                                                              TestMessageCollector(),
                                                              classpathFromClassloader(),
                                                              ScriptWithNoParam::class.qualifiedName!!,
